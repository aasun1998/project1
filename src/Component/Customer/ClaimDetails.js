...
export default function ClaimDetails() {
  ...
  useEffect(() => {
    instance
      ...
      .then(({ data }) => {
        console.log("customerdata", data?.claimDetails[0]);
        setAllLeadDetails(data?.claimDetails[0]);
        sigRef1.current.fromData(data?.claimDetails[0]?.customerSign);
        sigRef1.current.off();
        sigRef2.current.fromData(data?.claimDetails[0]?.authoritySign);
        sigRef2.current.off();
        sigRef3.current.fromData(data?.claimDetails[0]?.damageCarPicture1);
        sigRef3.current.off();
        sigRef4.current.fromData(data?.claimDetails[0]?.damageCarPicture2);
        sigRef4.current.off();
      })
      .catch((err) => {
        console.log("err", err);
