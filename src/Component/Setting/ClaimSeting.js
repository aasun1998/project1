...
const ClaimSeting = () => {
  ...
  useEffect(() => {
    instance
      .get(`/claim/setting/view`, {
        headers: { authorization: `Bearer ${localStorage.getItem("token$")}` },
      })
      .then(({ data }) => {
        console.log("policyData", data);
        setAllLeadDetails(data?.claimSettingData);
        setAllLeadFilter(data?.claimSettingData);
      })
      .catch((err) => {
        console.log("err", err);
      });
  ...
  const AddBlog = () => {
    history.push(`/addclaims`);
  };

  const onEdit = (value) => {
    setSetting(value);
    // console.log(setCustomer)
    history.push(`/cedit/${value._id}`);
  };
  ...
  const handelDelite = async (value, e) => {
    ...
    await instance
      .post(`/setting/claim/delite/${value._id}`)
      .then(() => {
