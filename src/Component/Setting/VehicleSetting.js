...
const VehicleSetting = () => {
  ...
  useEffect(() => {
    instance
      .get(`http://localhost:5000/api/vehicle/data`)
      .then(({ data }) => {
        console.log("vehicleData", data);
        setAllLeadDetails(data?.vehicleData);
        setAllLeadFilter(data?.vehicleData);
      })
      .catch((err) => {
        console.log("err", err);
      });
  ...
  const handelDelite = async (value, e) => {
    ...
    await instance
      .post(`/setting/vehicle/delite/${value._id}`)
      .then(() => {
