import React, { useEffect, useState } from "react";
import DashboardHeader from "../UDashboardHeader/DashboardHeader";
import { useHistory } from "react-router-dom";
import instance from "../../Instance";
import "./Profile.css";

const Profile = () => {
  const [oneview, setOneView] = useState(null);
  const history = useHistory();
  useEffect(() => {
    instance
      .get(`/one/user/view`, {
        headers: { authorization: `Bearer ${localStorage.getItem("token$")}` },
      })
      .then(({ data }) => {
        console.log("user", data);
        setOneView(data?.user);
      })
      .catch((err) => {
        console.log("err", err);
      });
  }, []);

  const ChangePassword = (oneview) => {
    history.push(`/userchange/${oneview._id}`);
  };

  return (
    <>
      <DashboardHeader />
      <div className="main-content-wrap sidenav-open d-flex flex-column">
        <button
          style={{ width: "200px" }}
          onClick={() => ChangePassword(oneview)}
          type="button"
          class="btn btn-success mr-sm-2 ml-auto"
        >
          Change Password
        </button>

        <div className="col-md-12 col-sm-12 text-center">
          {/* <img
            src="data:image/png;base64,"
            className="img-responsive img-circle tm-border text-center"
            alt="templatemo easy profile"
          /> */}
          <h1 className="bold">{oneview != null ? oneview.name : null}</h1>
          <h1 className="bold">{oneview != null ? oneview.email : null}</h1>
          <h1 className="bold">{oneview != null ? oneview.address : null}</h1>
          <h1 className="bold">{oneview != null ? oneview.phone : null}</h1>
          <h1 className="bold">{oneview != null ? oneview.suburb : null}</h1>
          <h1 className="bold">{oneview != null ? oneview.post : null}</h1>
        </div>
      </div>
    </>
  );
};

export default Profile;
